...
class Daftar_nilai extends MX_Controller
{
    ...
    public function import()
    {
        ...
        if ($this->M_daftar_nilai->insert_batch('tb_penilaian', $row)) {
            redirect(base_url('Kurikulum/Daftar_nilai'));
        } else {
            show_404();
        }
    }
    ...
    // Datatables
    public function nilai_datatables()
    {
        if ($this->input->is_ajax_request() == true) {

            $list = $this->M_daftar_nilai->get_datatables('v_penilaian');
            $data = array();
            $no = $_POST['start'];
            ...
            $output = array(
                "draw" => $_POST['draw'],
                "recordsTotal" => $this->M_daftar_nilai->count_all(),
                "recordsFiltered" => $this->M_daftar_nilai->count_filtered(),
                "data" => $data,
            );
